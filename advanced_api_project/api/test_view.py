...
class BookAPITestCase(APITestCase):
    ...
    def test_search_book_by_publication_year(self):
        response = self.client.get(reverse('book-list'), {'search': 2020})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)

    def test_order_book_by_title(self):
        response = self.client.get(reverse('book-list'), {'ordering': 'title'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        

    def test_order_book_by_author(self):
        response = self.client.get(reverse('book-list'), {'ordering': 'author'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
